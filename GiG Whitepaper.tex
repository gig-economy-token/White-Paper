...
The amount $\alpha$ of GET created and awarded to the ADA sender's wallet, will be an arbitrary constant $\phi$ (currently set to 1000 GET), divided by the block height $\ell$, starting from the first block since the GET creation smart contract gets published. That is,
...
\begin{enumerate}
  ...
  \item It provides an incentive to adopt the GiG Economy Token sooner than later. It can be compared to the Bitcoin Halving event. 
  \item After enough time, the conversion rate plateaus, thus providing a temporary price roof (in terms of ADA, at least). In detail, if the market price climbs higher than the GCSM cost of creating GET, market actors will prefer producing GET rather than buying it in the exchanges.
...
\subsection{Geolocation Control}
